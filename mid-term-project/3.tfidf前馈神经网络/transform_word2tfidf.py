...
for sentence in sentences:
    for num in set(sentence):   # transform into set to get rid of repeat words
        worddic_idf[num] = worddic_idf.get(num,0)+1
...
# 计算idf
for i in range(len(worddic_idf)):
    if worddic_idf[i] != 0:
        worddic_idf[i] = math.log10(sum(labels)/worddic_idf[i])

with open("idf.pkl","wb") as f:
    pickle.dump(worddic_idf,f)

#显示结果
print(worddic_idf)

#
def trans_n2tfidf(set):
    res=[]
    for sentence in set:
        tem=[]
        for num in sentence:
            tem.append(worddic_tf_pos[num]*worddic_idf[num])
            tem.append(worddic_tf_neg[num]*(-1)*worddic_idf[num])
        res.append(tem)
    return res


train_set=trans_n2tfidf(train_set)
validate_set=trans_n2tfidf(validate_set)
test_set=trans_n2tfidf(test_set)

with open("train_set_tfidf.pkl", "wb") as f:
    pickle.dump(train_set,f)
with open("validate_set_tfidf.pkl", "wb") as f:
    pickle.dump(validate_set,f)
with open("test_set_tfidf.pkl", "wb") as f:
