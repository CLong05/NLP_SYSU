...
# 读入数据
with open("train_set_tfidf.pkl", "rb") as f:
    x_train = pickle.load(f)
with open("validate_set_tfidf.pkl", "rb") as f:
    x_validate = pickle.load(f)
with open("test_set_tfidf.pkl", "rb") as f:
...
network = TwoLayerNet(input_size=200, hidden_size=100, output_size=2)
...
for i in range(iters_num):
    ...
    if i % iter_per_epoch == 0:
        ...
        # print(f"  train_rec: {format(train_rec, '.4f')}   validate_rec: {format(validate_rec, '.4f')} ")
        # print(f"  train_prec:{format(train_prec, '.4f')}  validate_prec:{format(validate_prec, '.4f')} ")
        if(len(validate_acc_list)==25):
            with open('save_tfidf_net.pkl', 'wb') as f:
        ...
        # print(f"acc:{train_acc} {test_acc}")
        # print(f"\tacc:{train_acc} {test_acc}  \trec:{train_rec} {test_rec}  \tprec:{train_prec} {test_prec}")
        # if test_acc>0.6730:
        #     res_acc = network.accuracy(x_test, t_test)
        #     print(res_acc)
